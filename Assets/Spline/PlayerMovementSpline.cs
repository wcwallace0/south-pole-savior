...
public class PlayerMovementSpline : MonoBehaviour
{
    ...
    public float slopeAccelRate = 0.5f;
    ...
    private void Accelerate() {
        ...
        float3 angle = spline.EvaluateTangent(distancePercentage);
        float slopeAngle = Mathf.Atan2(angle.y, angle.x) + Mathf.PI;
        // Debug.Log(angle);
        float accel = slopeAccelRate * (float) Math.Sin(slopeAngle);
        Debug.Log(accel);
        speed += accel;
